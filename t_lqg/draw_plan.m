function fig1=draw_plan(mean,cov,ctrl,params)
    fig1=figure(1);
    ...
    plot3(params.mean_init(1),params.mean_init(2),max(z),'ro','MarkerSize',10,'MarkerFaceColor','r');
...
%     plot3(params.robot_init(1),params.robot_init(2),max(z),'go','MarkerSize',10,'MarkerFaceColor','g');
    plot3(mean(1,:),mean(2,:),repmat(max(z),size(mean,2),1),'k','LineWidth',2);
    drawnow;
    legend('','initial belief','goal');
    
    subplot(3,4,4);
    time=0:params.time.dt:params.time.T;
    plot(time,mean(1,:),'LineWidth',2);
    hold on;
    plot(time,mean(2,:),'LineWidth',2);
    drawnow;
    legend('x','y');
    title('Subplot 2: x & y')
    
    subplot(3,4,8);
    plot(time,cov,'LineWidth',2);
    drawnow;
    legend('desired');
